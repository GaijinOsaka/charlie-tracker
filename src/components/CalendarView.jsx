...
function CalendarView({
  ...
  events.forEach((evt) => {
    ...
    const endDate = evt.event_end_date
      ? new Date(evt.event_end_date + "T00:00:00")
      : startDate;
        ...
        >
          {showDate && (
            <div className="cal-upcoming-date">
              ...
              <span className="cal-upcoming-month">
                {new Date(evt.event_date + "T00:00:00").toLocaleDateString(
                  "en-GB",
                  { month: "short" },
                )}
          ...
          <div className="cal-event-info">
            ...
            {evt.description && (
              <p className="cal-event-desc">{evt.description}</p>
            )}
            <div className="cal-event-meta">
              ...
              {evt.event_tags &&
                evt.event_tags.map((t) => (
                  <span key={t.tag} className="event-tag">
                    {t.tag}
                  </span>
                ))}
              {evt.messages && (
                <span className="event-source">
                  From: {evt.messages.sender_name || evt.messages.subject}
                </span>
              )}
              ...
              {evt.source_type === "manual" && (
                <span className="event-creator">
                  Created by:{" "}
                  {profiles?.[evt.created_by]?.display_name || "Unknown"}
                </span>
              )}
            </div>
          </div>
            ...
            <div className="event-message-header">
              ...
              <div className="event-message-meta-row">
                <span className="message-sender">
                  {evt.messages.sender_name || evt.messages.sender_email}
                </span>
                <span className="message-time">
                  {new Date(evt.messages.received_at).toLocaleString()}
                </span>
        ...
        {cells.map((day, i) => {
          ...
          return (
            <div
              ...
              title={
                onCreateEvent
                  ? "Click to select or double-click to create event"
                  : ""
              }
            >
              ...
              {hasEvents && (
                <div className="cal-event-summaries">
                  {dayEvents.slice(0, 2).map((e, j) => (
                    <div
                      key={j}
                      className={`cal-event-summary ${e.action_required ? "cal-event-action" : ""}`}
                    >
