...
const CATEGORIES = ['all', 'academic', 'admin', 'events', 'health', 'pastoral', 'general', 'other']
...
export default function DocumentBrowser() {
  ...
  async function handleBatchDelete() {
    ...
    if (count === 0) return
    if (!window.confirm(`Delete ${count} selected document${count > 1 ? 's' : ''}? This will remove files from storage and the database.`)) {
      return
    }
      ...
      <div className="doc-summary">
        ...
        {documents.filter(d => d.indexed_for_rag).length > 0 && (
          <span className="doc-summary-rag">
            {' '} &middot; {documents.filter(d => d.indexed_for_rag).length} indexed for RAG
          </span>
        )}
      </div>
